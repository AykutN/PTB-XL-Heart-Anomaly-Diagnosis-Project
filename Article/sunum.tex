...
\author{M. Yusuf Aykut\\İbrahim Abu Shawish\\Samet Ertuğrul Kurum\\Samet Baturay}
\date{Aralık 2025}
...
\begin{frame}{Literatürden Örnek Çalışma 1}
		extbf{[1]} PTB-XL ve Georgia ECG veri setlerinde, sol ventriküler hipertrofi (LVH) tespiti:
	\begin{itemize}
...
\begin{frame}{Literatürden Örnek Çalışma 2}
		extbf{[2]} Miyokardiyal Enfarktüs tespiti için klasik ML ve CNN karşılaştırması:
	\begin{itemize}
...
\begin{frame}{Literatürden Örnek Çalışma 3}
		extbf{[4]} PhysioNet/CinC 2020 Challenge, 12 derivasyonlu EKG ve XGBoost:
	\begin{itemize}
...
\begin{frame}{PTB-XL+ Veri Seti}
	\begin{itemize}
		...
		\item Bu sayede, modeller klinisyen etiketleri ile otomatik ifadeler arasında karşılaştırmalı olarak eğitilip değerlendirilebilir.
	\end{itemize}
\end{frame}
...
\begin{frame}{Figür 1: Veri Seti Genel Görünüm}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../eda/dataset_overview.png}}
\end{frame}
...
\begin{frame}{Ön İşleme: Genel Hatlar}
	\begin{itemize}
		...
		\item Veri sızıntısını önlemek için tüm dönüşümler eğitim/val/test ayrımından \emph{sonra} ve eğitim seti üzerinde fit edilmiştir.
	\end{itemize}
\end{frame}
...
\begin{frame}{Figür 2: Eksik Değer Analizi}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../figures/missing_values_analysis.png}}
\end{frame}

\begin{frame}{Eksik Veri Stratejisi}
	\begin{itemize}
		\item P dalgası ile ilişkili özniteliklerdeki eksiklik, MI ve STTC sınıflarında anlamlı biçimde daha sık görülmektedir.
...
\begin{frame}{Figür 3: Splitlere Göre Sınıf Dağılımı}
	\centering
	\resizebox{0.98\linewidth}{!}{\includegraphics{../figures/class_distribution_splits.png}}
\end{frame}
...
\begin{frame}{Öznitelik Seçimi: Random Forest Önem Skorları}
	\begin{itemize}
		...
		\item Her ağaçta kullanılan bölünmelerdeki bilgi kazançları toplanarak normalize edilmiş önem skorları elde edilmiştir.
...
\begin{frame}{Figür 4: Öznitelik Seçimi Özeti}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../preprocessing/feature_selection_analysis.png}}
\end{frame}
...
\begin{frame}{Model Seçimi: Random Forest}
	\begin{itemize}
		\item Bu çalışmada sınıflandırıcı olarak yalnızca Random Forest (RF) kullanılmıştır.
...
\begin{frame}{Dengesizlik Deney Tasarımı}
	\begin{itemize}
		...
		\item Senaryolar:
		\begin{itemize}
			\item \textbf{CW-50 / CW-100 / CW-200}: \texttt{class\_weight="balanced"} ile üç farklı öznitelik seti.
			\item \textbf{US-200}: Sadece eğitim setinde rastgele undersampling + Top-200 öznitelik.
		\end{itemize}
...
\begin{frame}{Kodlama Ortamı}
	\begin{itemize}
		...
		\item Bağımlılıklar \texttt{requirements.txt} ile yönetilmiştir: \texttt{pandas}, \texttt{numpy}, \texttt{scikit-learn}, \texttt{matplotlib}, \texttt{seaborn}, \texttt{tqdm} (opsiyonel: \texttt{wfdb}).
		...
		\item Tekrarlanabilirlik için veri bölme ve modelleme adımlarında sabit tohum (\texttt{random\_state=42}) kullanılmış; dönüşümler yalnızca eğitim setinde fit edilerek veri sızıntısı engellenmiştir.
...
\begin{frame}{Figür 5: ROC Eğrileri (Random Forest - Top-200)}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../results/roc_curves_RandomForest_top200.png}}
\end{frame}
...
\begin{frame}{Referanslar}
	...
	\begin{thebibliography}{9}
		\bibitem{ptbxl-scidata}
		P. Wagner, N. Strodthoff, R. Bousseljot, \emph{PTB-XL, a large publicly available electrocardiography dataset}, Scientific Data, 7, 154 (2020).

		\bibitem{physionet-ptbxl}
		PhysioNet, \emph{PTB-XL: A Large Annotated ECG Dataset}.\\
		\url{https://physionet.org/content/ptb-xl/}

		\bibitem{cardiodat}
		R. Bousseljot, D. Kreiseler, A. Schnabel, \emph{Nutzung der EKG-Signaldatenbank CARDIODAT der PTB über das Internet}, Biomedizinische Technik, 40(S1), 317--318 (1995).
	\end{thebibliography}
\end{frame}
