...
\subsection{Keşifsel Analiz: PCA ve Aykırı Değerler}

\begin{frame}{Figür: PCA Düzleminde Veri Dağılımı ve Aykırı Değerler}
	\centering
	\resizebox{0.8\linewidth}{!}{\includegraphics{../reports/outlier_analysis/pca_outliers_by_class.png}} %..\reports\outlier_analysis\pca_outliers_by_class.png
	\vspace{0.15cm}

	{\scriptsize
	\textit{Not: Renkler tanı üst sınıflarını, işaret tipi ise aykırı (x) / normal (o) örnekleri göstermektedir.}
	}
\end{frame}

\begin{frame}{PCA Grafiğinin Yorumu (2B Özet)}
	\small
	\begin{itemize}
		\item PCA, yüksek boyutlu EKG özniteliklerini iki ana bileşen üzerinde özetleyerek dağılımı görselleştirir \cite{jolliffe2016-pca-review}.
		\item Grafikte \textbf{PC1} yaklaşık \(\sim 0.21\), \textbf{PC2} yaklaşık \(\sim 0.13\) varyansı açıklamaktadır; ilk iki bileşen toplamda \(\sim 0.34\) varyansı yakalar.
		\item Sınıflar (NORM, MI, STTC, CD, HYP) 2B projeksiyonda belirgin biçimde \textbf{üst üste biner}:
		\begin{itemize}
			\item Bu durum, ayrımın tek/az sayıda lineer eksende değil; \textbf{çok boyutlu ve nonlineer} örüntülerde oluştuğunu düşündürür.
		\end{itemize}
		\item \textbf{Aykırı örnekler} (x ile işaretli) çoğunlukla bulutun dış bölgelerinde konumlanır:
		\begin{itemize}
			\item Ölçekleme/istatistiksel özetleri etkileyerek modele gürültü taşıyabilir,
			\item Ancak klinik açıdan \textbf{nadir ama gerçek} patoloji örnekleri olabileceği için doğrudan silinmemiş; sonraki adımlarda (varyans filtreleme, RF) sağlam (robust) öğrenme ile ele alınmıştır.
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Aykırı Değer Tespiti: Isolation Forest}
	Çalışmada kullanılan aykırı değer tespit yöntemi: \textbf{Isolation Forest (iForest)}:
	\begin{itemize}
		\item \textbf{Tablo 3}’te her tanı sınıfı için iForest ile aykırı değer analizi yapılmıştır.
		\item Her sınıf için yaklaşık \textbf{\%3.3--\%7.2} oranında aykırı değer tespit edilmiştir.
		\item \textbf{Temel fikir:} iForest, rastgele ağaçlar ile örneklerin \emph{izolasyon zorluğunu} ölçer ve her örnek için bir \textbf{anomali skoru} üretir.
		\item \textbf{Yorum:} Normal noktalar genellikle daha fazla bölünme ile izole edilirken, \textbf{daha kolay izole edilen} (daha kısa yol uzunluğuna sahip) örnekler aykırı değer olarak işaretlenir.
		\item \textbf{Eşikleme:} \texttt{contamination="auto"} ile yöntem, veri dağılımına göre \textbf{otomatik} bir eşik (threshold) belirler.
		\item iForest, \textbf{yüksek boyutlu} veri setlerinde etkinliği gösterilmiş, ölçeklenebilir bir aykırı değer tespit yaklaşımıdır.
	\end{itemize}
	\end{frame}
...
\begin{frame}{Figür 1: Eksik Değer Analizi}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../reports/missing_data_analysis/missing_values_barplot.png}}
\end{frame}

\begin{frame}{Figür 2: Eksik Değer Analizi}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../reports/missing_data_analysis/missingness_diagnosis_heatmap_paper.png}}
\end{frame}
...
\begin{frame}{Figür 3: Splitlere Göre Sınıf Dağılımı}
	\centering
	\resizebox{0.86\linewidth}{!}{\includegraphics{../reports/data_summary/class_distribution_splits.png}}
\end{frame}
...
\begin{frame}{Figür 3: Varyans Dağılımı}
	\centering
	\resizebox{0.95\linewidth}{!}{\includegraphics{../reports/feature_selection/variance_histogram.png}}
\end{frame}
...
\begin{frame}{Dengesizlik Deney Tasarımı}
	...
	\end{itemize}
\end{frame}
\begin{frame}{Model Değerlendirme Metrikleri (Tanımlar)}
	\small
	\begin{itemize}
		\item Her sınıf için one-vs-rest ikili senaryo kurulur: ilgili sınıf \emph{pozitif}, diğerleri \emph{negatif}.
		\item Karışıklık matrisi terimleri:
		\[
		\text{TP: doğru pozitif},\quad \text{FP: yanlış pozitif},\quad
		\text{FN: yanlış negatif},\quad \text{TN: doğru negatif}
		\]
		\item \textbf{Accuracy (Doğruluk)} — genel doğru sınıflandırma oranı (dengesiz veri varsa yanıltıcı olabilir):
		\[
		\text{Acc}=\frac{TP+TN}{TP+FP+FN+TN}
		\]
		\item \textbf{Precision (Kesinlik)} — “pozitif” dediğim örneklerin ne kadarı gerçekten pozitif? (yanlış alarm maliyeti yüksekse önemlidir):
		\[
		\text{Prec}=\frac{TP}{TP+FP}
		\]
		\item \textbf{Recall / Sensitivity (Duyarlılık)} — gerçek pozitiflerin ne kadarını yakaladım? (kaçırma maliyeti yüksekse önemlidir):
		\[
...
\begin{frame}{ROC-AUC, Eşik ve Çok Sınıflı Özetleme}
	\small
	\begin{itemize}
		\item \textbf{F1 skoru} — Precision ve Recall arasında denge (dengesiz sınıflarda sıklıkla tercih edilir):
		\[
		\text{F1}= \frac{2\,\text{Prec}\cdot\text{Rec}}{\text{Prec}+\text{Rec}}
		\]
		\item \textbf{ROC eğrisi}: eşik \(\tau\) değiştikçe \(\text{TPR}\) ve \(\text{FPR}\) değerlerinin izlediği eğri:
		\[
		\text{TPR}(\tau)=\frac{TP(\tau)}{TP(\tau)+FN(\tau)} \quad,\quad
		\text{FPR}(\tau)=\frac{FP(\tau)}{FP(\tau)+TN(\tau)}
		\]
		\item \textbf{AUC}: ROC eğrisinin altında kalan alan; eşikten bağımsız ayrıştırma gücü:
		\[
		\text{AUC}=\int_{0}^{1}\text{TPR}(\text{FPR})\,d(\text{FPR})
		\]
		
		\item Bu çalışmada metrikler doğrulama setinde hesaplanmış; sınıf bazlı eşikler \(\tau_c\) F1’i maksimize edecek şekilde taranmış ve en iyi kurulum test setinde raporlanmıştır.
	\end{itemize}
\end{frame}
...
%=================================
\section{Sonuçlar}
\begin{frame}{Sınıf Ağırlıkları vs. Undersampling}
	\begin{columns}[T,onlytextwidth]
		% --- Left: figure + short commentary ---
		\begin{column}{0.60\textwidth}
			\centering
			\includegraphics[width=\linewidth]{../reports/model_comparison/class_weights_distribution.png}

			{\scriptsize
			\textit{Not: Solda eğitim seti sınıf dağılımı, sağda \texttt{class\_weight="balanced"} ile hesaplanan ağırlıklar gösterilmektedir.}

			\begin{block}{Kısa yorum}
				\scriptsize
				\begin{itemize}
					\item \textbf{Class-weight:} Örnek sayısını değiştirmez; azınlık sınıfların hatasını daha fazla cezalandırarak öğrenmeyi dengeler.
					\item \textbf{Undersampling:} Çoğunluk sınıfından örnek \emph{atar}; veri boyutunu düşürür ve bilgi kaybı riski taşır.
				\end{itemize}
			\end{block}
			}
		\end{column}
		% --- Right: interpretation + formula ---
		\begin{column}{0.38\textwidth}
			\small
			\begin{itemize}
				\item \textbf{Sol grafik (dağılım):} Eğitim seti dengesizdir; \textbf{NORM} en büyük, \textbf{HYP} en küçük sınıftır.
				\item \textbf{Sağ grafik (ağırlıklar):} \texttt{balanced} yaklaşımı sınıf sıklığının tersiyle orantılı ağırlık verir:
				\[
					w_c \propto \frac{1}{n_c}
				\]
				\item Bu nedenle \textbf{NORM} için \(w<1\), \textbf{HYP} için \(w>1\) beklenir (kırmızı çizgi \(1.0\) referansıdır).
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{Genel Performans (Eğitim / Val) — Random Forest}
	\centering
	\small
	...
	\begin{tabular}{lccccc}
		\toprule
		\textbf{Kurulum} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{ROC-AUC} \\
		\midrule
	...
	\vspace{0.25cm}
	{\scriptsize \textbf{Not:} Ensemble kurulumunda yalnızca Recall yaklaşık \%78.6 olarak raporlanmıştır; diğer metrikler burada verilmemiştir.}
\end{frame}

\begin{frame}{SVM Sonuçları (Top-200, Class-Weight)}
	\centering
	\small
	...
	\begin{tabular}{lcccc}
		\toprule
		\textbf{Sınıf} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} & \textbf{ROC-AUC} \\
		\midrule
	...
	\vspace{0.25cm}
	{\scriptsize \textbf{Tablo 8.} SVM Top-200 CW modelinin sınıflara göre metrikleri}
\end{frame}

\begin{frame}{SVM (Top-200, CW) — Kısa Yorum}
	\small
	\begin{itemize}
		\item \textbf{NORM} sınıfı tüm metriklerde en yüksek skorlara sahiptir (\(F1=0.8602\), \(ROC\text{-}AUC=0.9435\)); bu durum sınıf dengesizliği ve NORM’un görece kolay ayrışması ile uyumludur.
		\item \textbf{HYP} sınıfı en düşük \(F1\) (\(0.6301\)) ve \textbf{Precision} (\(0.5714\)) değerlerini üretmiştir; azınlık sınıf olması nedeniyle modelin doğru pozitif üretmekte zorlandığını göstermektedir.
		\item \textbf{MI} ve \textbf{CD} sınıfları Precision/Recall açısından daha dengeli olup \(F1 \approx 0.748\) bandındadır.
		\item \textbf{STTC} için \textbf{Recall} (\(0.8464\)) yüksek, \textbf{Precision} (\(0.6632\)) düşüktür; model STTC’yi iyi yakalarken daha fazla yanlış pozitif üretiyor olabilir.
	\end{itemize}
\end{frame}
...
\begin{frame}{Figür 5: ROC Eğrileri (Random Forest - Top-200)}
	\centering
	\resizebox{0.70\linewidth}{!}{\includegraphics{../reports/model_comparison/roc_curves_final.png}}
\end{frame}
